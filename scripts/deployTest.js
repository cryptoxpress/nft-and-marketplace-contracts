...
async function main() {
   ...
   const xpress = await XPRESS_TOKEN.connect(deployer).deploy();
   ...
   await xpress.connect(deployer).initialize();
   console.log('Xpress Test Token Initialized');

   await cx_marketplace
      .connect(deployer)
      .initialize('1.0-Rinkeby', registry.address, forwarder.address);
   ...
   await cx_marketplace
      .connect(deployer)
      .setPaymentTokenAllowed(xpress.address, true); // approve test xpress token
